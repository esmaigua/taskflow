...
- Name: todos.spec.js >> Gestión de Tareas — Filtros >> El filtro "Completadas" solo muestra completadas
- Location: playwright\tests\todos.spec.js:178:3
...
Call log:
  - waiting for locator('[data-cy=todo-input]')
...
> 22  |   await page.locator('[data-cy=todo-input]').fill(text)
  ...
  118 |     const text = `Eliminar ${Date.now()}`
  119 |     await addTodo(page, text)
  120 |     // Hover para que aparezca el botón (CSS opacity: 0 → 1 en hover)
  121 |     const lastItem = page.locator('[data-cy=todo-item]').last()
  122 |     await lastItem.hover()
```